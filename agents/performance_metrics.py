...
class PerformanceTracker:
    ...
    def print_periodic_summary(self, round_num):
        ...
        net_balance = recent_ext_sold_value - recent_ext_bought_value
        ...
        print(f"     Bought from External Grid: {recent_ext_grid_bought:.1f} kWh (€{recent_ext_bought_value:.2f})")

        if net_balance > 0:
            print(f"     Net Balance: +€{net_balance:.2f} (export surplus) ✅")
        elif net_balance < 0:
            print(f"     Net Balance: -€{abs(net_balance):.2f} (import dependency) ⚠️")
        else:
            print(f"     Net Balance: €0.00 (self-sufficient) ✅")
        print("━" * 80 + "\n")
